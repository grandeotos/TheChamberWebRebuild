...
public class AccountDAO implements IAccountDAO {
    ...
    public int registerGamer(String username, String firstName, String lastName, String email) {
        String sql = "INSERT INTO `thechamber`.`account` (`rolid`, `username`, `firstName`, `lastName`, `email`, `password`, `create_time`) VALUES ('3', ?, ?, ?, ?, sha2(?,224), current_timestamp())";
        String sql2 = "SELECT COUNT(username) AS cuentas FROM account WHERE username = ?";
        try{
            Connection conexion = MySQLConnection.getConnection();
            PreparedStatement preparedStatement = conexion.prepareStatement(sql);
    ...
    public int updateApplicant(int appId, int action) {
        String sql = "UPDATE `thechamber`.`applicant` SET `reviewStatusId` = ? WHERE (`applicantId` = ?);";
        try{
            Connection conexion = MySQLConnection.getConnection();
            PreparedStatement preparedStatement = conexion.prepareStatement(sql);
